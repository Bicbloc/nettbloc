...
import { ArrowLeft, Check, X, Ban, RotateCcw, Trash2, Clock, Loader2, RefreshCw, Crown } from "lucide-react";
...
const GovernessAccessPage = () => {
  const navigate = useNavigate();
  const { user, isAuthenticated, loading: authLoading } = useAuth();
  const [requests, setRequests] = useState<GovernessRequest[]>([]);
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          id, governess_profile_id, hotel_id, hotel_code, status, requested_at,
          governess_profiles(name, email, phone),
          hotels(name)
        `)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('governess_requests_page')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'governess_access_requests' }, () => {
        loadRequests();
      })
      .subscribe();
  ...
  const handleApprove = async (request: GovernessRequest) => {
  ...
  const handleSuspend = async (request: GovernessRequest) => {
    setProcessingId(request.id);
    try {
      await supabase
        .from('governess_access_requests')
  ...
  const handleRevokeSuspension = async (request: GovernessRequest) => {
    setProcessingId(request.id);
    try {
      await supabase
        .from('governess_access_requests')
  ...
  const handleDelete = async (requestId: string) => {
    ...
    setProcessingId(requestId);
    try {
      await supabase.from('governess_access_requests').delete().eq('id', requestId);
        ...
        <header className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container mx-auto px-4 py-4 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold">Demandes d'accès - Gouvernantes</h1>
            </div>
                    ...
                    <TableBody>
                      {requests.map((request) => (
                        <TableRow key={request.id}>
                          <TableCell className="font-medium">{request.governess_profiles?.name || '-'}</TableCell>
                          <TableCell>{request.governess_profiles?.email || '-'}</TableCell>
                          <TableCell>{request.governess_profiles?.phone || '-'}</TableCell>
...
export default GovernessAccessPage;
