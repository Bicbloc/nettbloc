...
import { ArrowLeft, Check, X, Ban, RotateCcw, Trash2, Clock, Loader2, RefreshCw, UserPlus } from "lucide-react";
...
const HousekeeperAccessPage = () => {
  const navigate = useNavigate();
  const { user, isAuthenticated, loading: authLoading } = useAuth();
  const [requests, setRequests] = useState<HousekeeperRequest[]>([]);
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          housekeeper_profiles!inner(name, email, phone),
          hotels(name)
        `)
      ...
      const uniqueRequests = (data as any[] || []).filter((req) => {
        const key = `${req.housekeeper_profile_id}-${req.hotel_id}`;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('housekeeper_requests_page')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'housekeeper_access_requests' }, () => {
        loadRequests();
      })
      .subscribe();
  ...
  const handleApprove = async (request: HousekeeperRequest) => {
  ...
  const handleSuspend = async (request: HousekeeperRequest) => {
    setProcessingId(request.id);
    try {
      await supabase
        .from('housekeeper_access_requests')
  ...
  const handleRevokeSuspension = async (request: HousekeeperRequest) => {
    setProcessingId(request.id);
    try {
      await supabase
        .from('housekeeper_access_requests')
  ...
  const handleDelete = async (requestId: string) => {
    ...
    setProcessingId(requestId);
    try {
      await supabase.from('housekeeper_access_requests').delete().eq('id', requestId);
        ...
        <header className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container mx-auto px-4 py-4 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold">Demandes d'accès - Femmes de chambre</h1>
            </div>
                    ...
                    <TableBody>
                      {requests.map((request) => (
                        <TableRow key={request.id}>
                          <TableCell className="font-medium">{request.housekeeper_profiles.name}</TableCell>
                          <TableCell>{request.housekeeper_profiles.email}</TableCell>
                          <TableCell>{request.housekeeper_profiles.phone || '-'}</TableCell>
...
export default HousekeeperAccessPage;
