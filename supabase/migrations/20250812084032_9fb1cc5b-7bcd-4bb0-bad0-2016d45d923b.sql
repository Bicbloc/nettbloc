-- Create an authentication RPC to validate housekeeper access codes securely under RLS
-- This function validates both short (HTLXXX-1234) and long (HTLXXX-NAME-1234) codes
-- and can also resolve existing housekeepers and temporary sessions.
...
RETURNS TABLE(
  ...
  resolved_access_code text,
  code_source text -- 'access_code', 'housekeeper', 'session'
)
LANGUAGE plpgsql
SECURITY DEFINER
