-- Fix critical security vulnerability: Restrict public access to hotels table
-- Replace overly permissive policy with secure function-based access

-- Step 1: Create security definer function for hotel code validation only
CREATE OR REPLACE FUNCTION public.validate_hotel_code_exists(p_hotel_code text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  -- Only return true/false if hotel code exists, no other data exposed
  RETURN EXISTS (
    SELECT 1 FROM public.hotels 
    WHERE hotel_code = p_hotel_code
  );
END;
$$;

-- Step 2: Create function to get minimal hotel info for housekeeper authentication
CREATE OR REPLACE FUNCTION public.get_hotel_info_for_access_code(p_hotel_code text)
RETURNS TABLE(hotel_id uuid, hotel_name text, hotel_code text)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $$
BEGIN
  -- Only return minimal info needed for housekeeper access
  RETURN QUERY
  SELECT h.id, h.name, h.hotel_code
  FROM public.hotels h
  WHERE h.hotel_code = p_hotel_code;
END;
$$;

-- Step 3: Remove the dangerous public policy
DROP POLICY IF EXISTS "Allow hotel code lookup for housekeeper access requests" ON public.hotels;

-- Step 4: Create restricted policy for hotel code validation only
CREATE POLICY "Restricted hotel code validation" ON public.hotels
FOR SELECT USING (
  -- Only allow access via security definer functions
  -- This effectively blocks direct table access
  false
);

-- Step 5: Update existing authentication function to use secure approach
...
BEGIN
  ...
  FROM public.get_hotel_info_for_access_code(code_prefix);
