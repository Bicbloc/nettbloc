...
PRIORITÉ 3: INS ou PRO + client en arrivée UNIQUEMENT (pas de date/heure de départ) → TOUJOURS none (propre)
  - Exemple: "217 DBL INS - 10:42 2× Adultes Anja Odenthal 03/04/2026" → none (propre)
  - Le client arrive dans une chambre déjà propre (INS/PRO), rien à faire

PRIORITÉ 4: PRO ou INS ou DIR + client en DÉPART (date de départ et/ou heure de départ, PAS de date d'arrivée) → TOUJOURS a_blanc
  - Même si le statut dit PRO (propre), le client part donc nettoyage complet obligatoire
  - Exemple: "114 DBL PRO - 23/03/2026 2× Adultes JOHANE ADHORA BATTO FOPA 08:01" → a_blanc
  - La date seule + heure = départ, le client quitte la chambre

PRIORITÉ 5: Chambre vide (0 noms/clients)
  - INS/PRO sans client → none (propre, rien à faire)
  - SAL sans client → a_blanc

PRIORITÉ 6: SAL + 1 client en arrivée (nouveau client) → a_blanc

PRIORITÉ 7: Client en séjour avec heure d'arrivée visible (format Mews) → a_blanc peu importe le statut
